...
import {
  MapHeadersInput,
  MapHeadersInputSchema,
  MapHeadersOutput,
  MapHeadersOutputSchema,
} from '@/ai/schemas/mapping-schemas';
...
const mapHeadersFlow = ai.defineFlow(
  ...
  async (input): Promise<MapHeadersOutput> => {
    ...
    try {
      ...
      const validated = MapHeadersOutputSchema.parse(parsed);

      return validated;
    } catch (err) {
      console.error('Header mapping failed:', err);
...
export async function mapHeaders(
  input: MapHeadersInput
): Promise<MapHeadersOutput> {
