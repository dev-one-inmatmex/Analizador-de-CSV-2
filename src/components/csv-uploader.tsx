            ...
            <ScrollArea className="h-[450px] w-full rounded-md border">
              <Table>
                <TableHeader className="sticky top-0 bg-muted/50 backdrop-blur-sm">
                  <TableRow>
                    <TableHead className="w-[45%]">Columna en Archivo CSV</TableHead>
                    <TableHead className="w-[55%]">Mapear a Columna en '{selectedTableName}'</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {headers.map((csvHeader, csvIndex) => {
                    return (
                      <TableRow key={csvIndex}>
                        <TableCell className="font-medium text-foreground">
                          {csvHeader || <span className="text-muted-foreground italic">Columna sin nombre #{csvIndex + 1}</span>}
                        </TableCell>
                        <TableCell>
                          <Select
                            value={headerMap[csvIndex] || IGNORE_COLUMN_VALUE}
                            onValueChange={(newDbColumn) => handleMappingChange(csvIndex, newDbColumn)}
                          >
                            <SelectTrigger>
                            ...
                            <SelectContent>
                              ...
                              {tableColumns.map(col => (
                                <SelectItem
                                  key={col}
                                  value={col}
                                  disabled={usedDbColumns.has(col) && headerMap[csvIndex] !== col}
                                >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Alert variant={syncSummary.errorCount > 0 ? "destructive" : "default"}>
              {syncSummary.errorCount === 0 ? <CheckCircle className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" /> }
